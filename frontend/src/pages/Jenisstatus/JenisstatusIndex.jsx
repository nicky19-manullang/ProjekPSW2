...
function JenisstatusIndex() {
  ...
  const filteredData = data.filter(item =>
    item.jenis_status.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div style={{ fontFamily: "'Poppins', sans-serif", padding: "20px", backgroundColor: "#f8fafc", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        <h1 style={{ fontSize: "24px", fontWeight: "600", color: "#1e293b" }}>Daftar Jenis Status</h1>
        <button 
          onClick={() => navigate("/Jenisstatus-create")}
          style={{ 
      ...
      <div style={{ 
        ...
        <input
          type="text"
          placeholder="Cari jenis status atau keterangan..."
          style={{
            width: "100%",
        ...
        {!isLoading && currentItems.length > 0 && currentItems.map((item, index) => (
          ...
          >
            ...
            <div style={{ display: "flex", gap: "10px", justifyContent: "center" }}>
              <button 
                onClick={() => navigate(`/Jenisstatus-edit/${item.id}`)}
                style={{ 
                  color: "#3b82f6", 
                  background: "none", 
                  border: "none", 
