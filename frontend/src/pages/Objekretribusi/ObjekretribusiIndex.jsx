  ...
  return (
    <div style={{ fontFamily: "'Poppins', sans-serif", padding: "20px", backgroundColor: "#f8fafc", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        <h1 style={{ fontSize: "24px", fontWeight: "600", color: "#1e293b" }}>Objek Retribusi</h1>
        <button 
      ...
      <div style={{ backgroundColor: "white", borderRadius: "10px", boxShadow: "0 2px 10px rgba(0,0,0,0.05)", overflow: "hidden" }}>
        {/* Table Header */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", padding: "15px 20px", backgroundColor: "#4361ee", color: "white", fontWeight: "500" }}>
          <div>No</div>
          <div>Kode Objek</div>
          <div>Nama Objek</div>
          <div>Lokasi</div>
        ...
        {currentItems.length > 0 ? (
          currentItems.map((item, index) => (
            ...
            >
              ...
              <div>{item.objekRetribusi}</div>
              <div>{item.lokasi_objek_retribusi?.LokasiObjekRetribusi || '-'}</div>
              <div>{item.jenis_objek_retribusi?.jenisObjekRetribusi || '-'}</div>
              <div>{item.alamat}</div>
...
export default ObjekretribusiIndex;