...
function StatusIndex() {
  ...
  const filteredData = data.filter(item =>
    item.namaStatus.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (item.keterangan && item.keterangan.toLowerCase().includes(searchTerm.toLowerCase())) ||
    (item.jenisStatus && item.jenisStatus.namaJenisStatus.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  return (
    <div style={{ fontFamily: "'Poppins', sans-serif", padding: "20px", backgroundColor: "#f8fafc", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        <h1 style={{ fontSize: "24px", fontWeight: "600", color: "#1e293b" }}>Daftar Status</h1>
        <button 
      ...
      <div style={{ 
        ...
        <input
          type="text"
          placeholder="Cari status, keterangan, atau jenis status..."
          style={{
            width: "100%",
      ...
      <div style={{ backgroundColor: "white", borderRadius: "10px", boxShadow: "0 2px 10px rgba(0,0,0,0.05)", overflow: "hidden" }}>
        ...
        {!isLoading && currentItems.length > 0 && currentItems.map((item, index) => (
          ...
          >
            ...
            <div>{item.jenisStatus?.namaJenisStatus || "-"}</div>
